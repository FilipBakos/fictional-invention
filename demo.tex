...
\author{
 ...
 \affaddr{uman-Computer Interaction Lab :: Computer Science :: Institute for Advanced Computer Studies}\\
 \affaddr{College Park, Maryland 20740}\\
\affaddr{\{aq, bederson\}@cs.umd.edu}}
...
\say{\textit{…a paradigm for utilizing human processing power to solve problems that computers cannot yet solve.}}

FOTO2

This seems compatible with definitions given elsewhere by von Ahn (co-author on the first one) and others (the rest):

\say{\textit{…the idea of using human effort to perform tasks that computers cannot yet perform, usually in an enjoyable manner.}}

\say{\textit{…a new research area that studies the process of channeling the vast internet population to perform tasks or provide data towards solving difficult problems that no known efficient computer algorithms can yet solve.}}

\say{\textit{…a technique that makes use of human abilities for computation to solve problems.}}

\say{\textit{…a technique to let humans solve tasks, which cannot be solved by computers. [54]}}

\say{\textit{A computational process that involves humans in certain steps… [73]}}

\say{\textit{…systems of computers and large numbers of humans that work together in order to solve problems that could not be solved by either computers or humans alone [50]}}

\say{\textit{…a new area of research that studies how to build systems, such as simple casual games, to collect annotations from human users. [34]}}
